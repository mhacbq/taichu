...
- [ ] [UI] 塔罗结果页“保存记录”按钮看起来可正常点击，但 `saveTarotResult()` 提交 `spread_type` 时读取了未定义变量 `drawnSpread.value`；用户可能在点击保存后直接落入失败提示，且界面事前没有任何异常信号 - 塔罗占卜页 / 结果操作区 - 建议改为统一使用已存在的牌阵状态变量，并在保存前补充必需字段校验。
...
- [ ] [UI] 六爻历史记录弹窗虽然维护了 `historyLoading`、`historyError`、`historyLoaded`，但模板里始终只渲染 `history` 列表；接口加载中、失败或无记录时弹窗会接近空白，用户很难判断是“还在加载”还是“没有内容” - 六爻占卜页 / 历史记录弹窗 - 建议补齐 loading、error、empty 三种状态，并保留重试入口。
- [ ] [UI] 合婚免费预览页的升级提示区被 `v-if="!isLoading"` 包裹，点击“解锁详细报告”后整块说明和按钮会立刻消失；用户只能看到页面切换前的一段空档，不清楚系统是否真的在处理中 - 合婚页 / 免费预览升级区 - 建议保留升级卡片并改为按钮 loading、文案进度提示或骨架态，而不是整个区域直接隐藏。
