...
### 🟡 中优先级（运营体验问题）
- [ ] [运营] Dashboard 仍缺少整页错误态；`loadAllData()` 在统计、趋势、实时动态请求失败时会继续保留默认 0 值、`尚未加载` 和空列表，只弹一次通用提示，运营很容易把接口异常误判成真实经营数据 - 后台首页 Dashboard - 建议补齐页内错误态、重试入口与只读保护。
- [ ] [运营] 用户列表、黄历/神煞、充值订单、VIP 订单等核心列表页在加载失败时仍主要依赖 toast 报错，页面主体会继续显示空表或旧数据，缺少和系统配置页一致的显式错误态/只读保护，运营很难判断“暂无数据”还是“接口没加载成功” - 用户管理 / 内容管理 / 订单管理 - 建议统一接入页内错误态、禁用写操作并提供重试按钮。
...
### 🔴 高优先级（逻辑错误/准确性问题）
- [ ] [占卜] 八字流年分析把未来五年写成 `0~4` 序号，`best_year/worst_year` 也只返回索引值，用户无法对应实际年份且大运流年判断失真 - 八字排盘 - `fortune_analysis.years[*].year` 当前直接取数组键名，前台看到的是 `0/1/2/3/4` 而不是 `2026~2030`，会把流年吉凶和应期判断整体带偏。
...
- [ ] [占卜] 合婚详细报告在免费预览成功后仍因历史表字段不匹配报错，付费用户无法获取完整合婚结论 - 合婚配对 - premium 调用返回 `Field 'male_birth' doesn't have a default value`，当前控制器写入 `male_birth_date/female_birth_date`，与运行库实际字段口径不一致。
- [ ] [占卜] 每日运势返回的黄历主数据为空，页面总干支与黄历字段无法校验，个性化分析却另给一套 `todayGanZhi`，前后口径不一致 - 每日运势 - `ganzhi` 为空，`dayGanzhi/monthGanzhi/zhiri/jishen/xiongsha/shichen` 大量缺失，但 `personalized.todayGanZhi` 仍返回 `辛卯`，用户无法确认今日运势是按哪套黄历底盘推导。

### 🟡 中优先级（体验问题）
- [ ] [占卜] 合婚免费预览虽然能出分，但详细报告链路在确认解锁后才暴露数据库错误，用户会先看到可购买承诺、再收到失败提示，付费信任感受损 - 合婚配对 - 建议在生成详细报告前先做历史表结构健康检查或预写入校验，把“可解锁”与“可成功生成”保持一致。
