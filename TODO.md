...
### 🔴 高优先级（功能性问题）
- [ ] [UI] 六爻页的定价加载、历史加载和主流程失败仍主要依赖瞬时 toast，页面本身缺少持续的页内错误承接与重试入口 - 六爻占卜 / 表单页与历史记录 - 建议分别为定价区、历史区和占卜主流程补齐 error/empty/retry 状态，失败时同步清空旧数据并禁用高风险操作，避免用户误判为“暂无数据”或重复提交。
- [ ] [UI] 合婚免费预览后的详细报告解锁失败只弹出“积分不足，请先充值”等瞬时提示，结果页没有页内恢复链路，用户无法就地完成充值、刷新价格或再次确认解锁 - 八字合婚 / 免费预览结果页 - 建议在升级卡片内保留失败说明，并补充“去充值”“刷新价格”“重新解锁”入口，把失败处理从一次性 toast 改成可恢复的页内流程。

### 🟡 中优先级（体验问题）
- [ ] [UI] 八字结果页深度工具在价格异常时会把按钮文案改成“请先刷新价格信息”，但结果区没有对应刷新入口，用户进入结果页后会卡在操作死胡同 - 八字排盘 / 结果页深度工具区 - 建议在结果区补充“刷新价格/重试加载”按钮，或把按钮直接改成可执行的恢复动作，而不是只显示提示文案。
- [ ] [UI] 登录后的“今日陪伴”悬浮控件在全站固定显示，移动端也没有给底部操作区预留安全空间，容易遮挡结果页底部按钮和正文阅读 - 全局 `App.vue` / floating-companion - 建议按页面场景收敛显示范围，为主内容区补底部安全区，或在小屏改成更轻量的吸边入口/抽屉，减少跨页遮挡。
- [ ] [UI] 每日运势在数据尚未返回或加载失败时，顶部日期卡仍会先渲染，容易出现“农历 / 公历为空”的半成品页面 - 每日运势 / 顶部日期区 - 建议把日期卡并入 loading/error 分支，未拿到日期前显示骨架屏或占位文案，避免空白壳子先于真实内容出现。
