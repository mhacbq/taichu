...
- [ ] [占卜] 每日运势个性化今日干支仍算错，导致十神关系与建议整体跑偏 - 每日运势 - `/api/daily/fortune` 返回 `todayGanZhi = 庚戌`，但外部黄历（2026-03-17）为 `庚寅日`；当前“比劫/吉”与幸运建议建立在错误日柱上，会直接误导用户对当日旺衰与行事节奏的判断。
...
### 🟢 低优先级（专业性优化）
- [ ] [占卜] 八字简版解读文案仍有明显模板拼接病句，削弱专业感 - 八字排盘 - 测试样例返回 `作为阳金你，你如同刀剑利器一般`、`基于你的金你特性` 等不通顺表述，容易让用户怀疑命理解读是生硬模板而非专业分析。
- [ ] [占卜] 塔罗解读仍混入英文术语且综合结论过度套话，和韦特体系的语境不够贴合 - 塔罗牌占卜 - 三张牌解读直接出现 `Enemy Dignity`、`Mutual Dignity`，凯尔特十字结尾统一落到“接受命运的指引”，既不够本土化，也削弱了牌阵关系与建议的针对性。
