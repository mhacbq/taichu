...
- [ ] [中] 充值订单页筛选当前存在“接口 200 但前端筛不动 / 搜索直接炸页”的真实运营风险：`2026-03-19` 在同一管理员登录态下，`GET /api/admin/payment/orders?page=1&limit=10` 返回 `total=4`、`user_id=1,1,2,4`；但加上页面实际发送的 `user_id=1` 后，返回仍是 `total=4`、`user_id=1,1,2,4`，说明 `backend/app/controller/admin/Payment.php::getOrders()` 目前并没有消费这个筛选参数，后台“用户ID”筛选点了等于没点。进一步用页面同源搜索参数 `keyword=R202403200002` 请求 `GET /api/admin/payment/orders?page=1&limit=5&keyword=R202403200002`，接口直接返回 `HTTP 500` HTML 错页；而 `admin/src/views/payment/orders.vue` 的 `refreshPage()` / `handleLoadFailure()` 会在 `loadOrders()` rejected 时切到 `createReadonlyErrorState()`，页面会从订单表格直接退化成“充值订单加载失败”只读错误卡，不是单纯空结果。
