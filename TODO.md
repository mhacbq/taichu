...
#### 🔴 高优先级（功能性/安全问题）
- [ ] [2026-03-17 23:50] **后端Config.php响应格式不统一** - backend/app/controller/Config.php第22-26行、第36-40行使用`json(['code' => 0])`而非`$this->success()` - 建议统一使用BaseController的success/error方法，统一返回code=200
- [ ] [2026-03-17 23:50] **后端Admin.php管理员信息获取错误** - backend/app/controller/Admin.php第42-44行使用`$this->request->user`获取用户信息，但后台管理应使用`$this->request->adminUser` - 建议修改为`$adminUser = $this->request->adminUser ?? []`
- [ ] [2026-03-17 23:50] **后端Upload.php文件扩展名验证风险** - backend/app/controller/Upload.php第413行使用`getOriginalExtension()`获取客户端提供的扩展名，存在安全风险 - 建议使用`getExtension()`获取真实扩展名防止伪造
- [ ] [2026-03-17 23:50] **后端Auth.php异常信息泄露** - backend/app/controller/Auth.php第76行用户创建失败时直接返回`$e->getMessage()`，可能泄露敏感信息 - 建议记录日志后返回通用错误消息
- [ ] [2026-03-17 23:50] **管理端Dashboard响应码判断混乱** - admin/src/views/dashboard/index.vue第131行使用`res.code === 200`，但第148、159、170、181行使用`res.code === 0`，判断标准不一致 - 建议统一修改为`res.code === 200`
