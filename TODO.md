...
## 🔵 E. 2026-03-26 新一轮巡查发现问题

> 来源：2026-03-26 代码审查（实际读取文件验证）

### P0 级 - 核心功能缺陷

- [x] **[P0][功能]** 流年运势前后端参数不匹配 — [已修复]
  - 📍 复现步骤：访问 `/yearly-fortune` → 填写出生日期时间 → 点击"开始解析" → 后端实际忽略前端传入的 `birthDateTime` 参数
  - 🔍 来源：代码审查
  - 💡 预期行为：前端 `useYearlyFortune.js` 发送 `{ birthDateTime, gender }` 参数，但后端 `Fortune::yearlyFortune()` 只读取 `bazi_id`，没有 `birthDateTime` 参数处理逻辑。当用户没有历史八字记录且未设置 profile 出生日期时，会返回 422 错误"您尚未设置出生日期"，而不是使用页面填写的日期。应当支持前端直接传入出生日期进行计算，或在 UI 上明确告知用户需要先完成八字排盘。

### P1 级 - 功能不完整

- [x] **[P1][管理端]** 任务调度 - 脚本"日志"按钮点击无响应 — [已修复]
  - 📍 复现步骤：管理端 → 任务调度 → 脚本管理列表 → 点击"日志"按钮 → 无任何反应
  - 🔍 来源：代码审查（`admin/src/views/task/list.vue` 第 297 行 `handleViewLog` 函数体为空）
  - 💡 预期行为：点击"日志"按钮应跳转到任务执行日志页面（`/task/logs`），并按脚本ID过滤

- [x] **[P1][管理端]** 任务调度 - 执行日志页面无真实数据 — [已修复]
  - 📍 复现步骤：管理端 → 任务调度 → 执行日志 → 页面始终显示空列表
  - 🔍 来源：代码审查（`admin/src/views/task/logs.vue` 中 `loadLogs()` 函数用 `setTimeout` 模拟，没有真实 API 调用）
  - 💡 预期行为：应调用后端 `GET /tasks/logs` 接口（路由已注册，`Admin/taskLogs`），展示真实的任务执行日志

### P2 级 - 代码质量

- [x] **[P2][代码]** 前台 `console.error` 残留（非结构化日志） — [已修复]
  - 📍 复现步骤：打开浏览器控制台，触发相关功能报错时会看到原始 error 输出
  - 🔍 来源：代码审查（`useYearlyFortune.js` 第 67、140 行；`useTarot.js` 第 453 行；`useQiming.js` 第 155 行）
  - 💡 预期行为：`console.error` 应替换为结构化日志或静默处理，避免在生产环境泄露错误信息
