...
const loadList = async () => {
  ...
  try {
    ...
    const res = await request.get('/maodou/liuyao-manage', { params })
    if (res.code === 200) {
...
const loadStats = async () => {
  try {
    const res = await request.get('/maodou/liuyao-manage/stats')
    if (res.code === 200) {
...
const handleView = async (row) => {
  ...
  try {
    const res = await request.get(`/maodou/liuyao-manage/${row.id}`)
    if (res.code === 200) {
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('确定要删除这条六爻记录吗？', '确认删除', { type: 'warning' })
    const res = await request.delete(`/maodou/liuyao-manage/${row.id}`)
    if (res.code === 200) {
...
const handleBatchDelete = async () => {
  ...
  try {
    ...
    const res = await request.post('/maodou/liuyao-manage/batch-delete', { ids: selectedIds.value })
    if (res.code === 200) {
...
const getMethodLabel = (method) => {
  const map = { time: '时间起卦', number: '数字起卦', manual: '手动摇卦' }
  ...
  <div class="admin-manage-page">
    ...
    <div class="search-form">
      <el-form :inline="true" :model="searchForm">
        <el-form-item label="关键词">
          <el-input v-model="searchForm.keyword" placeholder="搜索问题/结果" clearable />
        </el-form-item>
    ...
    <div class="table-container">
      <el-table v-loading="loading" :data="list" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="question" label="所问之事" min-width="180" show-overflow-tooltip />
        <el-table-column label="起卦方式" width="110">
        ...
        <el-table-column label="AI分析" width="90">
          <template #default="{ row }">
            <el-tag :type="row.is_ai ? 'success' : 'info'" size="small">
              {{ row.is_ai ? '是' : '否' }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <!-- 详情弹窗 -->
    <el-dialog v-model="detailVisible" title="六爻记录详情" width="700px">
      <div v-loading="detailLoading">
        <template v-if="detailData">
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item label="所问之事" :span="2">{{ detailData.question }}</el-descriptions-item>
            <el-descriptions-item label="起卦方式">{{ getMethodLabel(detailData.method) }}</el-descriptions-item>
            <el-descriptions-item label="AI分析">{{ detailData.is_ai ? '是' : '否' }}</el-descriptions-item>
            <el-descriptions-item label="占卜时间" :span="2">{{ detailData.created_at }}</el-descriptions-item>
          </el-descriptions>
