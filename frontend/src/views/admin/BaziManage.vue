...
const loadList = async () => {
  ...
  try {
    ...
    const res = await request.get('/maodou/bazi-manage', { params })
    if (res.code === 200) {
...
const loadStats = async () => {
  try {
    const res = await request.get('/maodou/bazi-manage/stats')
    if (res.code === 200) {
...
const handleView = async (row) => {
  ...
  try {
    const res = await request.get(`/maodou/bazi-manage/${row.id}`)
    if (res.code === 200) {
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('确定要删除这条八字记录吗？', '确认删除', { type: 'warning' })
    const res = await request.delete(`/maodou/bazi-manage/${row.id}`)
    if (res.code === 200) {
...
const handleBatchDelete = async () => {
  ...
  try {
    ...
    const res = await request.post('/maodou/bazi-manage/batch-delete', { ids: selectedIds.value })
    if (res.code === 200) {
...
<template>
  <div class="admin-manage-page">
    ...
    <div class="search-form">
      <el-form :inline="true" :model="searchForm">
        <el-form-item label="关键词">
          <el-input v-model="searchForm.keyword" placeholder="搜索姓名/八字" clearable />
        </el-form-item>
    ...
    <div class="table-container">
      <el-table v-loading="loading" :data="list" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="gender" label="性别" width="70">
          <template #default="{ row }">{{ row.gender === 'male' || row.gender === 1 ? '男' : '女' }}</template>
        </el-table-column>
        <el-table-column prop="birthday" label="出生时间" min-width="150" />
        <el-table-column prop="created_at" label="排盘时间" width="180" />
    ...
    <!-- 详情弹窗 -->
    <el-dialog v-model="detailVisible" title="八字记录详情" width="700px">
      <div v-loading="detailLoading">
        <template v-if="detailData">
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item label="性别">{{ detailData.gender === 'male' || detailData.gender === 1 ? '男' : '女' }}</el-descriptions-item>
            <el-descriptions-item label="出生时间" :span="2">{{ detailData.birthday }}</el-descriptions-item>
            <el-descriptions-item label="排盘时间" :span="2">{{ detailData.created_at }}</el-descriptions-item>
          </el-descriptions>
          <div v-if="detailData.result || detailData.bazi_data" class="detail-result">
            <h4>排盘结果</h4>
            <div class="result-content" v-html="detailData.result || JSON.stringify(detailData.bazi_data, null, 2)"></div>
          </div>
        </template>
      </div>
    </el-dialog>
