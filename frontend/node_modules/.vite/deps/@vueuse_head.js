import { Kn as ref, Nt as onDeactivated, Ot as nextTick, _n as watchEffect, gn as watch, jt as onBeforeUnmount, kt as onActivated, mn as version, nr as unref, nt as defineComponent, ut as getCurrentInstance, xt as inject } from "./vue.runtime.esm-bundler-lRCzm0bN.js";
//#region node_modules/packrup/dist/index.mjs
function unpackToArray(input, options) {
	const unpacked = [];
	const kFn = options.resolveKeyData || ((ctx) => ctx.key);
	const vFn = options.resolveValueData || ((ctx) => ctx.value);
	for (const [k, v] of Object.entries(input)) unpacked.push(...(Array.isArray(v) ? v : [v]).map((i) => {
		const ctx = {
			key: k,
			value: i
		};
...
var ObjectArrayEntries = /* @__PURE__ */ new Set([
	"og:image",
	"og:video",
	"og:audio",
	"twitter:image"
...
function normaliseTag(tagName, input, e, normalizedProps) {
	...
	for (const k of TagConfigKeys) {
		...
		if (val !== void 0) {
			if (!(k === "innerHTML" || k === "textContent" || k === "children") || TagsWithInnerContent.has(tag.tag)) tag[k === "children" ? "innerHTML" : k] = val;
			delete tag.props[k];
		}
	}
	if (tag.props.body) {
...
function normaliseStyleClassProps(key, v) {
	const sep = key === "class" ? " " : ";";
	if (v && typeof v === "object" && !Array.isArray(v)) v = Object.entries(v).filter(([, v2]) => v2).map(([k, v2]) => key === "style" ? `${k}:${v2}` : k);
...
function nestedNormaliseProps(props, virtual, keys, startIndex) {
	for (let i = startIndex; i < keys.length; i += 1) {
		...
		if (props[k] instanceof Promise) return props[k].then((val) => {
			props[k] = val;
		...
		if (!virtual && !TagConfigKeys.has(k)) {
			...
			if (v === "true" || v === "") props[k] = isDataKey ? "true" : true;
			else if (!props[k]) if (isDataKey && v === "false") props[k] = "false";
			else delete props[k];
		}
	}
}
...
function normaliseEntryTags(e) {
	...
	for (const k in input) {
		...
		const v = input[k];
		if (v === void 0 || !ValidHeadTags.has(k)) continue;
		if (Array.isArray(v)) {
			for (const props of v) tagPromises.push(normaliseTag(k, props, e));
			continue;
		}
...
var NetworkEvents = /* @__PURE__ */ new Set([
	"onload",
	"onerror",
	"onabort",
	"onprogress",
...
//#endregion
//#region node_modules/@unhead/dom/dist/index.mjs
async function renderDOMHead(head, options = {}) {
	...
	head._domUpdatePromise = new Promise(async (resolve) => {
		...
		function trackCtx({ id, $el, tag }) {
			...
			if (tag._eventHandlers) for (const k in tag._eventHandlers) {
				if (!Object.prototype.hasOwnProperty.call(tag._eventHandlers, k)) continue;
				if ($el.getAttribute(`data-${k}`) !== "") {
					(tag.tag === "bodyAttrs" ? dom.defaultView : $el).addEventListener(k.substring(2), tag._eventHandlers[k].bind($el));
...
//#endregion
//#region node_modules/unhead/dist/index.mjs
var UsesMergeStrategy = /* @__PURE__ */ new Set([
	"templateParams",
	"htmlAttrs",
	"bodyAttrs"
]);
var DedupePlugin = defineHeadPlugin({ hooks: {
	"tag:normalise": ({ tag }) => {
		if (tag.props.hid) {
...
var ValidEventTags = /* @__PURE__ */ new Set([
	"script",
	"link",
	"bodyAttrs"
]);
var EventHandlersPlugin = defineHeadPlugin((head) => ({ hooks: {
	"tags:resolve": (ctx) => {
		for (const tag of ctx.tags) {
...
var HashKeyedPlugin = defineHeadPlugin({ hooks: { "tag:normalise": ({ tag }) => {
...
var SortPlugin = defineHeadPlugin({ hooks: { "tags:resolve": (ctx) => {
	for (const tag of ctx.tags) {
...
var TemplateParamsPlugin = defineHeadPlugin((head) => ({ hooks: {
	"tags:resolve": (ctx) => {
		const { tags } = ctx;
		let templateParams;
		for (let i = 0; i < tags.length; i += 1) {
			if (tags[i].tag !== "templateParams") continue;
...
var TitleTemplatePlugin = defineHeadPlugin({ hooks: { "tags:resolve": (ctx) => {
	const { tags } = ctx;
	let titleTag;
...
var XSSPlugin = defineHeadPlugin({ hooks: { "tags:afterResolve": (ctx) => {
...
//#endregion
//#region node_modules/@unhead/vue/dist/shared/vue.ziyDaVMR.mjs
var Vue3 = version[0] === "3";
function resolveUnref(r) {
	return typeof r === "function" ? r() : unref(r);
}
function resolveUnrefHeadInput(ref) {
	if (ref instanceof Promise || ref instanceof Date || ref instanceof RegExp) return ref;
...
var VueReactivityPlugin = defineHeadPlugin({ hooks: { "entries:resolve": (ctx) => {
...
function createHead$1(options = {}) {
	...
	const head = /* @__PURE__ */ createHead$2(options);
...
//#endregion
//#region node_modules/@unhead/vue/dist/shared/vue.-sixQ7xP.mjs
...
var coreComposableNames = ["injectHead"];
var unheadVueComposablesImports = { "@unhead/vue": [...coreComposableNames, ...composableNames] };
...
//#endregion
//#region node_modules/@unhead/vue/dist/polyfill.mjs
...
//#endregion
//#region node_modules/@unhead/vue/dist/components.mjs
...
function createHead(initHeadObject, options) {
	const legacyHead = polyfillAsVueUseHead(createHead$1(options || {}));
