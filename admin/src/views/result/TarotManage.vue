  ...
  <div class="app-container">
    <el-card class="search-form" shadow="never">
      <el-form :model="queryForm" inline>
        ...
        <el-form-item label="牌阵类型">
          <el-select v-model="queryForm.spread_type" placeholder="全部牌阵" clearable>
            <el-option label="圣三角牌阵" value="single" />
            <el-option label="凯尔特十字" value="celtic_cross" />
            <el-option label="二选一" value="two_choices" />
          </el-select>
        </el-form-item>
        <el-form-item label="关键词">
          <el-input v-model="queryForm.keyword" placeholder="搜索问题或解读" clearable />
        </el-form-item>
      ...
      <template v-else>
        ...
        >
          ...
          <el-table-column prop="spread_type" label="牌阵类型" width="120">
            <template #default="{ row }">
              <el-tag type="primary">{{ getSpreadTypeName(row.spread_type) }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="cards" label="抽选牌数" width="100" align="center">
            <template #default="{ row }">
              {{ row.cards ? JSON.parse(row.cards).length : 0 }}
            </template>
          </el-table-column>
    ...
    <el-dialog v-model="detailDialog.visible" title="塔罗测算详情" width="900px" destroy-on-close>
      <el-descriptions v-if="detailDialog.data" :column="1" border>
        ...
        <el-descriptions-item label="问题">{{ detailDialog.data.question }}</el-descriptions-item>
        <el-descriptions-item label="牌阵类型">
          <el-tag type="primary">{{ getSpreadTypeName(detailDialog.data.spread_type) }}</el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="抽选的牌">
          <div v-if="detailDialog.data.cards" class="cards-container">
            <div v-for="(card, index) in JSON.parse(detailDialog.data.cards)" :key="index" class="card-item">
              <span class="card-name">{{ card.name }}</span>
              <span class="card-position" :class="{ 'reversed': card.is_reversed }">
        ...
        <el-descriptions-item label="解读结果">
          <div class="result-content">{{ detailDialog.data.interpretation || '暂无' }}</div>
        </el-descriptions-item>
