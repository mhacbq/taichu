...
1. **管理端仪表盘路由跳转失效修复** (高优先级)
   - 文件: `admin/src/views/dashboard/index.vue`
   - 修复: 补齐缺失的 `useRouter` 导入及 `router` 实例定义，确保待处理反馈跳转功能正常。

2. **全站移动端导航图标丢失修复** (中优先级)
   - 文件: `frontend/src/App.vue`
   - 修复: 在图标导入列表中增加 `Home` 图标，解决移动端侧边栏首页图标无法显示的问题。

3. **八字排盘组件代码清理与优化** (中优先级)
   - 文件: `frontend/src/views/Bazi.vue`
   - 修复: 移除了 7 个未使用的图标导入；修复了五行分布图表的 `--target-width` CSS 变量缺失问题，使渲染动画生效。

4. **八字排盘深色模式视觉兼容性优化** (低优先级)
   - 文件: `frontend/src/views/Bazi.vue`
   - 修复: 将 `.rizhu-tag` 的硬编码颜色 `#fff` 替换为全局变量 `var(--text-primary)`，提升深色背景下的可读性。

5. **合婚页面冗余代码清理** (低优先级)
   - 文件: `frontend/src/views/Hehun.vue`
   - 修复: 移除了页面底部的重复媒体查询样式块，减小组件体积。

### TODO.md 清理
- 移除了已在代码中实现的“神煞管理API未实现”任务。
- 移除了已在代码中实现的“SEO统计图表功能未实现”任务。

### Git提交信息
- 提交时间: 2026-03-17 19:15
- 提交信息: fix-frontend-multiple-issues-20260317-1915
- 状态: 已推送至 origin master
