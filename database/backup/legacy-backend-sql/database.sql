...
CREATE TABLE `liuyao_records` (
  `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `user_id` int(11) unsigned NOT NULL COMMENT '用户ID',
  `question` varchar(255) NOT NULL COMMENT '占卜问题',
  `yao_result` varchar(6) NOT NULL COMMENT '六爻结果（6位数字，每位0/1/2/3）',
  `gua_code` varchar(2) NOT NULL COMMENT '卦象代码',
  `gua_name` varchar(50) NOT NULL COMMENT '卦名',
  `gua_ci` text COMMENT '卦辞',
  `yao_ci` text COMMENT '爻辞',
  `interpretation` text COMMENT '解读',
  `ai_analysis` text COMMENT 'AI深度分析',
  `is_ai_analysis` tinyint(1) DEFAULT 0 COMMENT '是否使用AI分析',
  `consumed_points` int(11) DEFAULT 0 COMMENT '消耗积分',
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_user_id` (`user_id`),
  KEY `idx_gua_code` (`gua_code`),
  KEY `idx_created_at` (`created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='六爻占卜记录表';

-- 六爻卦象基础数据表
DROP TABLE IF EXISTS `liuyao_gua`;
CREATE TABLE `liuyao_gua` (
  `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `gua_code` varchar(2) NOT NULL COMMENT '卦象代码（二进制）',
  `gua_name` varchar(50) NOT NULL COMMENT '卦名',
  `gua_ci` text COMMENT '卦辞',
  `xiang_ci` text COMMENT '象辞',
  `tuan_ci` text COMMENT '彖辞',
  `yao_1` text COMMENT '初爻',
  `yao_2` text COMMENT '二爻',
  `yao_3` text COMMENT '三爻',
  `yao_4` text COMMENT '四爻',
  `yao_5` text COMMENT '五爻',
  `yao_6` text COMMENT '上爻',
  `wuxing` varchar(10) DEFAULT '' COMMENT '五行属性',
  `gong` varchar(20) DEFAULT '' COMMENT '所属宫',
  `description` text COMMENT '卦象描述',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_gua_code` (`gua_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='六爻卦象基础表';

-- 插入六十四卦基础数据
INSERT INTO `liuyao_gua` (`gua_code`, `gua_name`, `gua_ci`, `wuxing`, `gong`, `description`) VALUES
('00', '坤为地', '元亨，利牝马之贞。君子有攸往，先迷后得主，利西南得朋，东北丧朋。安贞，吉。', '土', '坤宫', '坤卦象征大地，代表柔顺、包容、承载万物'),
('01', '地雷复', '亨。出入无疾，朋来无咎。反复其道，七日来复，利有攸往。', '土', '坤宫', '复卦象征回复、复兴，表示事物发展到一定阶段会重新开始'),
('02', '地水师', '贞，丈人，吉无咎。', '水', '坎宫', '师卦象征军队、战争，代表组织纪律和团队精神'),
('03', '地泽临', '元，亨，利，贞。至于八月有凶。', '金', '坤宫', '临卦象征临近、观察，表示居高临下审视事物'),
('04', '地风升', '元亨，用见大人，勿恤，南征吉。', '木', '震宫', '升卦象征上升、进步，表示事业蒸蒸日上'),
('05', '地天泰', '小往大来，吉亨。', '金', '坤宫', '泰卦象征通达、安泰，表示天地交合万物通泰'),
('06', '地火明夷', '利艰贞。', '火', '坎宫', '明夷卦象征光明受损，表示在困难中保持正直'),
('07', '地山谦', '亨，君子有终。', '土', '兑宫', '谦卦象征谦虚，表示谦虚谨慎能带来好的结果'),
('10', '雷地豫', '利建侯行师。', '木', '震宫', '豫卦象征愉悦、安乐，表示心情愉悦但要适度'),
('11', '震为雷', '亨。震来虩虩，笑言哑哑。震惊百里，不丧匕鬯。', '木', '震宫', '震卦象征震动、雷，表示突发事件和警醒'),
('12', '雷水解', '利西南，无所往，其来复吉。有攸往，夙吉。', '木', '震宫', '解卦象征解脱、解决，表示困难得以解除'),
('13', '雷泽归妹', '征凶，无攸利。', '金', '兑宫', '归妹卦象征嫁娶，表示女子出嫁'),
('14', '雷风恒', '亨，无咎，利贞，利有攸往。', '木', '震宫', '恒卦象征恒久，表示持之以恒方能成功'),
('15', '雷天大壮', '利贞。', '木', '震宫', '大壮卦象征强盛，表示力量强大但要守正'),
('16', '雷火丰', '亨，王假之，勿忧，宜日中。', '火', '坎宫', '丰卦象征丰盛，表示事业达到顶峰'),
('17', '雷山小过', '亨，利贞，可小事，不可大事。飞鸟遗之音，不宜上宜下，大吉。', '金', '兑宫', '小过卦象征小有过失，表示小的偏差'),
('20', '水地比', '吉。原筮元永贞，无咎。不宁方来，后夫凶。', '水', '坤宫', '比卦象征亲近、辅助，表示团结互助'),
('21', '水雷屯', '元亨，利贞，勿用有攸往，利建侯。', '水', '坎宫', '屯卦象征初生、困难，表示事物刚开始的艰难'),
('22', '坎为水', '习坎，有孚，维心亨，行有尚。', '水', '坎宫', '坎卦象征水、险陷，表示重重困难和危险'),
('23', '水泽节', '亨。苦节不可贞。', '水', '坎宫', '节卦象征节制，表示适度节制才能亨通'),
('24', '水风井', '改邑不改井，无丧无得，往来井井。汔至亦未繘井，羸其瓶，凶。', '水', '震宫', '井卦象征水井，表示恒定不变和滋养'),
('25', '水天需', '有孚，光亨，贞吉。利涉大川。', '水', '坤宫', '需卦象征等待，表示需要耐心等待时机'),
('26', '水火既济', '亨，小利贞，初吉终乱。', '水', '坎宫', '既济卦象征完成，表示事情已经成功'),
('27', '水山蹇', '利西南，不利东北；利见大人，贞吉。', '土', '兑宫', '蹇卦象征艰难，表示前进困难'),
('30', '泽地萃', '亨。王假有庙，利见大人，亨，利贞。用大牲吉，利有攸往。', '金', '兑宫', '萃卦象征聚集，表示人才和资源的聚集'),
('31', '泽雷随', '元亨利贞，无咎。', '金', '震宫', '随卦象征随从，表示顺应时势'),
('32', '泽水困', '亨，贞，大人吉，无咎，有言不信。', '水', '坎宫', '困卦象征困窘，表示处于困境'),
('33', '兑为泽', '亨，利贞。', '金', '兑宫', '兑卦象征喜悦、泽，表示心情愉悦'),
('34', '泽风大过', '栋桡，利有攸往，亨。', '木', '震宫', '大过卦象征大的过失，表示过度的行为'),
('35', '泽天夬', '扬于王庭，孚号，有厉，告自邑，不利即戎，利有攸往。', '金', '坤宫', '夬卦象征决断，表示果断决策'),
('36', '泽火革', '巳日乃孚，元亨利贞，悔亡。', '火', '坎宫', '革卦象征变革，表示改革创新'),
('37', '泽山咸', '亨，利贞，取女吉。', '金', '兑宫', '咸卦象征感应，表示心灵相通'),
('40', '风地观', '盥而不荐，有孚颙若。', '木', '乾宫', '观卦象征观察，表示仔细观察事物'),
('41', '风雷益', '利有攸往，利涉大川。', '木', '巽宫', '益卦象征增益，表示受益和成长'),
('42', '风水涣', '亨。王假有庙，利涉大川，利贞。', '水', '坎宫', '涣卦象征涣散，表示分散和化解'),
('43', '风泽中孚', '豚鱼吉，利涉大川，利贞。', '金', '艮宫', '中孚卦象征诚信，表示内心诚信'),
('44', '巽为风', '小亨，利攸往，利见大人。', '木', '巽宫', '巽卦象征风、顺，表示顺从和进入'),
('45', '风天小畜', '亨。密云不雨，自我西郊。', '木', '巽宫', '小畜卦象征小有蓄积，表示积累阶段'),
('46', '风火家人', '利女贞。', '火', '离宫', '家人卦象征家庭，表示家庭和睦'),
('47', '风山渐', '女归吉，利贞。', '木', '艮宫', '渐卦象征渐进，表示循序渐进'),
('50', '天地否', '否之匪人，不利君子贞，大往小来。', '金', '乾宫', '否卦象征闭塞，表示天地不交'),
('51', '天雷无妄', '元亨利贞。其匪正有眚，不利有攸往。', '金', '巽宫', '无妄卦象征无妄之灾，表示意外'),
('52', '天水讼', '有孚，窒惕，中吉，终凶。利见大人，不利涉大川。', '水', '离宫', '讼卦象征争讼，表示纠纷和诉讼'),
('53', '天泽履', '履虎尾，不咥人，亨。', '金', '艮宫', '履卦象征履行、实践，表示谨慎行事'),
('54', '天风姤', '女壮，勿用取女。', '金', '乾宫', '姤卦象征遇合，表示意外相遇'),
('55', '乾为天', '元亨利贞。', '金', '乾宫', '乾卦象征天，代表刚健、创造、领导力'),
('56', '天火同人', '同人于野，亨。利涉大川，利君子贞。', '火', '离宫', '同人卦象征团结，表示志同道合'),
('57', '天山遁', '亨，小利贞。', '金', '乾宫', '遁卦象征退避，表示暂时退让'),
('60', '火地晋', '康侯用锡马蕃庶，昼日三接。', '火', '坤宫', '晋卦象征晋升，表示事业上升'),
('61', '火雷噬嗑', '亨。利用狱。', '火', '离宫', '噬嗑卦象征咬合，表示解决问题'),
('62', '火水未济', '亨，小狐汔济，濡其尾，无攸利。', '火', '离宫', '未济卦象征未完成，表示事情尚未成功'),
('63', '火泽睽', '小事吉。', '火', '艮宫', '睽卦象征背离，表示意见不合'),
('64', '火风鼎', '元吉，亨。', '火', '离宫', '鼎卦象征鼎器，表示变革和革新'),
('65', '火天大有', '元亨。', '火', '乾宫', '大有卦象征大有收获，表示丰收'),
('66', '离为火', '利贞，亨。畜牝牛，吉。', '火', '离宫', '离卦象征火、光明，表示依附和光明'),
('67', '火山旅', '小亨，旅贞吉。', '火', '离宫', '旅卦象征旅行，表示出门在外'),
('70', '山地剥', '不利有攸往。', '土', '乾宫', '剥卦象征剥落，表示衰败'),
('71', '山雷颐', '贞吉，观颐，自求口实。', '土', '巽宫', '颐卦象征颐养，表示养生之道'),
('72', '山水蒙', '亨。匪我求童蒙，童蒙求我。初筮告，再三渎，渎则不告。利贞。', '土', '离宫', '蒙卦象征蒙昧，表示启蒙教育'),
('73', '山泽损', '有孚，元吉，无咎，可贞，利有攸往。曷之用，二簋可用享。', '土', '艮宫', '损卦象征减损，表示有所失'),
('74', '山风蛊', '元亨，利涉大川。先甲三日，后甲三日。', '木', '巽宫', '蛊卦象征腐败，表示整治弊端'),
('75', '山天大畜', '利贞，不家食吉，利涉大川。', '土', '艮宫', '大畜卦象征大蓄积，表示积累丰富'),
('76', '山火贲', '亨，小利有所往。', '火', '艮宫', '贲卦象征装饰，表示文饰美化'),
('77', '艮为山', '艮其背，不获其身，行其庭，不见其人，无咎。', '土', '艮宫', '艮卦象征山、止，表示停止和稳重');
...
('六爻占卜解读', 'liuyao_interpret', 'liuyao', '你是一位精通六爻占卜的资深易学大师，擅长通过卦象解读事物发展规律。请用专业但易懂的语言为用户提供六爻解读。',
'请解读以下六爻卦象：\n问题：{question}\n本卦：{gua_name}（{gua_code}）\n卦辞：{gua_ci}\n动爻：{yao_changes}\n请提供详细的卦象分析和建议。', '六爻占卜解读提示词');
...
('feature_liuyao_enabled', '1', 'bool', '六爻占卜功能开关', 'feature', 1, 10),
('feature_invite_enabled', '1', 'bool', '邀请好友功能开关', 'feature', 1, 11),
...
('points_cost_liuyao', '15', 'int', '六爻占卜积分', 'points_cost', 1, 10),
-- 限时优惠配置
