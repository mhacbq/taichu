...
-- ============================================================
-- 如果字段不存在则添加（使用存储过程兼容处理）
SET @dbname = DATABASE();
SET @tablename = 'tc_points_record';
SET @columnname = 'action';
SET @columndef = 'VARCHAR(100) NOT NULL DEFAULT "" COMMENT "动作描述" AFTER `user_id`';
...
-- 如果不存在则执行添加
SET @sql = IF(@exists = 0, @sql, 'SELECT "Column already exists"');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- ============================================================
-- 2. 用户表 tc_user - 添加缺失字段
-- ============================================================
-- invite_code
SET @columnname = 'invite_code';
SET @columndef = 'VARCHAR(20) DEFAULT "" COMMENT "邀请码" AFTER `status`';
SELECT COUNT(*) INTO @exists FROM information_schema.columns 
WHERE table_schema = @dbname AND table_name = 'tc_user' AND column_name = @columnname;
SET @sql = IF(@exists = 0, CONCAT('ALTER TABLE `tc_user` ADD COLUMN `', @columnname, '` ', @columndef), 'SELECT "invite_code already exists"');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @columndef = 'INT(11) UNSIGNED DEFAULT 0 COMMENT "被谁邀请" AFTER `invite_code`';
SELECT COUNT(*) INTO @exists FROM information_schema.columns 
WHERE table_schema = @dbname AND table_name = 'tc_user' AND column_name = @columnname;
SET @sql = IF(@exists = 0, CONCAT('ALTER TABLE `tc_user` ADD COLUMN `', @columnname, '` ', @columndef), 'SELECT "invited_by already exists"');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @columndef = 'TINYINT(1) DEFAULT 0 COMMENT "是否VIP: 0否 1是" AFTER `invited_by`';
SELECT COUNT(*) INTO @exists FROM information_schema.columns 
WHERE table_schema = @dbname AND table_name = 'tc_user' AND column_name = @columnname;
SET @sql = IF(@exists = 0, CONCAT('ALTER TABLE `tc_user` ADD COLUMN `', @columnname, '` ', @columndef), 'SELECT "is_vip already exists"');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- vip_expire_at
SET @columnname = 'vip_expire_at';
SET @columndef = 'DATETIME DEFAULT NULL COMMENT "VIP过期时间" AFTER `is_vip`';
SELECT COUNT(*) INTO @exists FROM information_schema.columns 
WHERE table_schema = @dbname AND table_name = 'tc_user' AND column_name = @columnname;
SET @sql = IF(@exists = 0, CONCAT('ALTER TABLE `tc_user` ADD COLUMN `', @columnname, '` ', @columndef), 'SELECT "vip_expire_at already exists"');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
